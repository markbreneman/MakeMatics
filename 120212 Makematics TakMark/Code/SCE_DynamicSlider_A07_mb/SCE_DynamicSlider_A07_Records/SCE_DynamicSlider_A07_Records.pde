...
SeamCarving carver;
//ArrayObject[] seamPoints;
ArrayList<PVector> seam;
ArrayList<PVector> seamPoints;
...
void setup() {
  makeArray();///LOAD IMAGE,CREATE SEAM CARVER, ENERGY IMAGE....SEE VOID TAB..JUST A FUNCTION.
  seamPoints = new ArrayList<PVector>();//<--this arraylist is the master history.
  ...
  for (int i=0; i<img.height; i++) {
    for (int j=0; j<img.width; j++) {
      ...
      seamPoints.add(loc, tempPV);
      tempPoints.add(loc, tempPV);
    }
  }
  history = createWriter("ConferenceRoomMapped01-edited.txt");
}
...
void keyReleased() {
  ...
  if (key=='n' && img.width>= 4) {
    ...
    for (int i=1; i<seam.size(); i++) {
      ...
      seamPoints.set(int(oPosition.x), tempPV);
...
//////////////////////////  SAVE DATA EXTERNALLY  //////////////////////////////////   
  ...
  if (key=='s') {
    ...
    for (int i=0; i<seamPoints.size(); i++) {
      history.println(seamPoints.get(i).x+","+seamPoints.get(i).y);
    }
    history.flush();
    history.close();
    exit();
